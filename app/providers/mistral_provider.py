...
class MistralProvider(BaseAIProvider):
    def __init__(self, api_key: Optional[str] = None, model_name: Optional[str] = None):
        ...
        self._model_name = model_name or settings.MISTRAL_MODEL
    ...
    async def generate_response(
        ...
        for idx, m in enumerate(messages):
            ...
            # If this is the last user message and image_data is attached, construct multimodal payload
            if idx == len(messages) - 1 and role == "user" and image_data:
                # If image_data is already a data URI or URL
        ...
        try:
            ...
            raw_err = str(e)
            is_quota = any(kw in raw_err.lower() for kw in ("429", "rate_limit", "quota", "too many requests", "resource_exhausted"))
            return ProviderResponse(
                content="",
                model_name=self.model_name,
                provider_name=self.provider_name,
